...
// Deriv New API public market-data WebSocket.
// No App ID or login is required for public ticks.
...
async function connectPriceFeed(){
  try {
    ...
    console.error('Could not create Deriv public WebSocket:', e);
    setFeedStatus(false);
    scheduleReconnect();
    return;
  }
  ...
  priceSocket.onopen = () => {
    ...
    SYMBOLS.forEach(symbol => {
      priceSocket.send(JSON.stringify({ ticks: symbol, subscribe: 1 }));
    });
  };
  ...
  priceSocket.onmessage = (event) => {
    ...
    if (data.error) console.error('Deriv API error:', data.error.message || data.error);
    if (Array.isArray(data.errors)) {
      data.errors.forEach(err => console.error('Deriv API error:', err.message || err));
    }
  };
